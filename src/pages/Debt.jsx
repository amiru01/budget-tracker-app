    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="dashboard-card p-5 transition hover:shadow-md">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-400">{debt.category} &middot; {debt.frequency}</p>
        </div>
        <div className="text-right">
          <p className="text-lg font-bold text-white">{formatCurrency(debt.remainingBalance)}</p>
          <p className="text-xs text-slate-500">of {formatCurrency(debt.totalAmount)}</p>
        </div>
      </div>
      ...
      {debt.interestRate > 0 && <p className="mt-2 text-xs text-slate-500">Interest: {debt.interestRate}%</p>}
...
export default function Debt() {
  ...
  if (loading) {
    return <div className="flex min-h-[50vh] items-center justify-center px-4 py-10">
      <div className="dashboard-card px-4 py-3 text-sm font-bold text-slate-300">Loading debts…</div>
    </div>
  }
    ...
    <div className="space-y-6">
      <motion.div {...quickFade} transition={{ ...quickFade.animate.transition, delay: 0 }}>
        <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="font-display mt-2 text-3xl font-extrabold tracking-[-0.03em] text-white sm:text-4xl">Debt Payoff</h1>
            <p className="mt-1 text-sm text-slate-400">Track, manage, and eliminate your debt.</p>
          </div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <motion.div {...quickFade} transition={{ ...quickFade.animate.transition, delay: 0.08 }} className="dashboard-card p-5">
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-400">Active Debts</p>
          <p className="font-display mt-2 text-2xl font-extrabold text-white">{activeDebts.length}</p>
        </motion.div>
        <motion.div {...quickFade} transition={{ ...quickFade.animate.transition, delay: 0.1 }} className="dashboard-card p-5">
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-400">Remaining</p>
        ...
        <motion.div {...quickFade} transition={{ ...quickFade.animate.transition, delay: 0.12 }} className="dashboard-card p-5">
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-400">Paid So Far</p>
      ...
      <div className="dashboard-card p-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="font-display text-lg font-bold text-white">Overall Progress</h2>
            <p className="text-xs text-slate-400">{formatCurrency(totalPaid)} paid of {formatCurrency(totalOriginal)}</p>
          </div>
          <span className="text-2xl font-bold text-white">{totalOriginal > 0 ? ((totalPaid / totalOriginal) * 100).toFixed(0) : 0}%</span>
        </div>
      ...
      {paidDebts.length > 0 && (
        <motion.div {...quickFade} transition={{ ...quickFade.animate.transition, delay: 0.22 }} className="dashboard-card p-6">
          <h2 className="font-display text-lg font-bold text-white">Paid Off 🎉</h2>
          <div className="mt-3 space-y-2">
            {paidDebts.map((debt) => (
              <div key={debt.id} className="flex items-center justify-between rounded-xl bg-emerald-400/10 p-3 ring-1 ring-emerald-400/20">
                <div>
                  <p className="font-semibold text-emerald-200">{debt.name}</p>
                  <p className="text-xs text-emerald-300/70">{debt.category}</p>
                </div>
                <p className="font-bold text-emerald-200">{formatCurrency(debt.totalAmount)}</p>
              </div>
            ))}
          </div>
        </motion.div>
