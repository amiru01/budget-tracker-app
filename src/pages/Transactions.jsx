  ...
  if (loading) {
    return (
      <div className="flex min-h-[50vh] items-center justify-center px-4 py-10">
        <div className="dashboard-card px-4 py-3 text-sm font-bold text-ink-secondary">Loading transactions…</div>
      </div>
    )
  }
      ...
      <motion.div {...quickFade} transition={{ ...quickFade.animate.transition, delay: 0 }}>
        <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="font-display mt-2 text-3xl font-extrabold tracking-[-0.03em] text-ink sm:text-4xl">Transactions</h1>
            <p className="mt-1 text-sm text-ink-secondary">Net worth overview with all your income and expense entries.</p>
          </div>
      ...
      <motion.div {...quickFade} transition={{ delay: 0.1 }}>
        <section className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <article className="dashboard-card p-5">
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-ink-secondary">Total Income</p>
            <p className="font-display mt-2 text-2xl font-extrabold text-emerald-400">{formatCurrency(totalIncomeAll)}</p>
            <p className="mt-1 text-xs text-ink-tertiary">{incomes.length} entries</p>
          </article>
          <article className="dashboard-card p-5">
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-ink-secondary">Total Expenses</p>
            <p className="font-display mt-2 text-2xl font-extrabold text-rose-400">{formatCurrency(totalExpensesAll)}</p>
            <p className="mt-1 text-xs text-ink-tertiary">{expenses.length} entries</p>
          </article>
          <article className="dashboard-card p-5">
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-ink-secondary">Net Balance</p>
            <p className={`font-display mt-2 text-2xl font-extrabold ${netBalance >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
              {formatCurrency(netBalance)}
            </p>
            <p className="mt-1 text-xs text-ink-tertiary">{netBalance >= 0 ? 'Positive' : 'Negative'}</p>
          </article>
        </section>
      </motion.div>
      ...
      <motion.div {...quickFade} transition={{ delay: 0.2 }}>
        <section className="dashboard-card p-6">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="font-display text-xl font-bold tracking-tight text-ink">Transaction History</h2>
              <p className="mt-1 text-sm font-medium leading-6 text-ink-secondary">All your income and expenses with payment accounts.</p>
            </div>
            <motion.span {...badgePop} className="rounded-full bg-surface-secondary px-3 py-1 text-xs font-bold text-ink-secondary ring-1 ring-border-subtle">
          ...
          <ul className="mt-6 space-y-3">
            {allTransactions.length === 0 ? (
              <motion.li initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="rounded-3xl bg-surface-secondary px-6 py-6 text-center ring-1 ring-border-subtle shadow-lg shadow-slate-950/10">
                <div className="mx-auto max-w-sm">
                  ...
                  <p className="mt-4 text-lg font-bold text-ink">No entries yet</p>
                  <p className="mt-2 text-sm font-medium text-ink-secondary">Add your first income or expense to start tracking.</p>
