...
export default function ReportCard({ label, value, icon, trend, variant = 'default' }) {
  ...
  const iconStyles = {
    default: 'bg-ink-secondary/10 text-ink-secondary',
    success: 'bg-emerald-400/10 text-emerald-300',
    danger: 'bg-rose-400/10 text-rose-600',
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          ...
          <p className="font-display mt-2 text-2xl font-extrabold tracking-tight text-ink sm:text-3xl">
            {value}
          </p>
          ...
          {trend && (
            <div className="mt-2 flex items-center gap-2">
              {trend.direction === 'up' && (
                <span className="inline-flex items-center gap-1 rounded-full bg-rose-400/10 px-2 py-0.5 text-xs font-semibold text-rose-600">
                  <span>↑</span>
              ...
              {trend.direction === 'stable' && (
                <span className="inline-flex items-center gap-1 rounded-full bg-ink-secondary/10 px-2 py-0.5 text-xs font-semibold text-ink-secondary">
                  <span>→</span>
                  <span>Stable</span>
                </span>
              )}
